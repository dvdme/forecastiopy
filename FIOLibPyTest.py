...
fio = ForecastIO.ForecastIO(apikey,
                            units_url=ForecastIO.ForecastIO.UNITS_SI,
                            lang_url=ForecastIO.ForecastIO.LANG_PORTUGUESE,
                            latitude=Alcatraz[0], longitude=Alcatraz[1])

cur = FIOCurrently.FIOCurrently(fio)

print cur.get().keys()
for item in cur.get().keys():
    print item + ' : ' + str(cur.get()[item])
...
print alerts.get()
print alerts.get_alert(1)['title']
print alerts.alerts_count()

print fio.X_Forecast_API_Calls
